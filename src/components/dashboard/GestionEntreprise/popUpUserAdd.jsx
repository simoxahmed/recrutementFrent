import React, { useEffect, useState } from 'react';
import authService from '../../../services/authService';
import Cookies from 'js-cookie';

const PopUpUserAdd = ({ isOpen, onClose }) => {
  const [selectedUser, setSelectedUser] = useState('');
  const [selectedEntreprise, setSelectedEntreprise] = useState('');
  const [usersList, setUsers] = useState([]);


  const handleListUser= async()=>{
    try {
        const response = await authService.AllUser();
        console.log(response.data);
        const [arrayFavorate] = Object.values(response.data);
        setUsers(arrayFavorate)
        console.log(usersList);
    } catch (error) {
        console.error('Error fetching data:', error);
    }
}
useEffect(() => {
  handleListUser();
}, []); // Empty dependency array to run once on component mount

  const handleUserChange = (event) => {
    setSelectedUser(event.target.value);
  };

  const handleEntrepriseChange = (event) => {
    setSelectedEntreprise(event.target.value);
  };

  const onSubmitForm= async(e)=>{
    e.preventDefault();
    try {
      const response = await authService.addUser(selectedUser,selectedEntreprise);
      console.log(response.data.post_success);
      const msg = response.data.post_success;
      Cookies.set('msgUserAdd',msg)
      onClose()
    } catch (error) {
      console.error('Error creating demand:', error.response.data);
      // handle error response
    }
  }



  return (
    <div className={`fixed inset-0 z-50 overflow-y-auto ${isOpen ? '' : 'hidden'}`}>
      <div className="flex items-center justify-center min-h-screen px-4">
        <div className="fixed inset-0 transition-opacity" aria-hidden="true">
          <div className="absolute inset-0 bg-gray-500 opacity-75"></div>
        </div>
        <form onSubmit={onSubmitForm} className="relative bg-white rounded-lg p-8 max-w-md w-full mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold">Select Options</h2>
            <div onClick={onClose} className="text-gray-500 hover:text-gray-700 focus:outline-none">
              <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path>
              </svg>
            </div>
          </div>
          <div className="mb-6">
            <label htmlFor="users" className="block text-gray-700 font-medium mb-2">Select User:</label>
            <select id="users" value={selectedUser} onChange={handleUserChange}
              className="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-500">
                <option value=''>Select User</option>
              { usersList && usersList.map((user) => (
                <option value={user.id}>{user.nm+' '+user.np}</option>
              ))}

            </select>
          </div>
          <div className="mb-6">
            <label htmlFor="entreprise" className="block text-gray-700 font-medium mb-2">Select Entreprise:</label>
            <select id="entreprise" value={selectedEntreprise} onChange={handleEntrepriseChange}
              className="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:border-blue-500">
              <option value="">Select Entreprise</option>
              {usersList && usersList.map((user) => (
                user.entreprise_id && user.entreprise_id.map((item) => (
                  item.entreprise_id !== null ? (
                  <option key={item.entreprise_id} value={item.entreprise_id}>
                    {item.entreprise_name}
                  </option>
                  ) : null
                ))
              ))}
            </select>
          </div>
          <div className="flex justify-end">
            <div onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-700 font-semibold rounded-md mr-2 hover:bg-gray-300 focus:outline-none">
              Cancel
            </div>
            <button type='submit' className="px-4 py-2 bg-blue-500 text-white font-semibold rounded-md hover:bg-blue-600 focus:outline-none">
              Submit
            </button>
          </div>
        </form>
      </div>
    </div>
  );
};

export default PopUpUserAdd;
